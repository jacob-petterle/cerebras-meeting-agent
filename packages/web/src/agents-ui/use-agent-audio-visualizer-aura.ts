...
const DEFAULT_TRANSITION: ValueAnimationTransition = { duration: 0.6, ease: 'easeOut' };
/**
 * Tuned down from the stock Aura: a SLOW, easeInOut breathe instead of a fast mirror strobe, so the
 * pulsing states don't flash. Paired with much lower brightness peaks in the switch below — the shader
 * blooms hard above ~1.4, which is what made it flash to white.
 */
  ...
  useEffect(() => {
    switch (state) {
      ...
      case 'disconnected':
        ...
        animateBrightness(0.85, DEFAULT_TRANSITION);
        return;
      case 'listening':
      case 'pre-connect-buffering':
        ...
        animateBrightness([1.0, 1.35], DEFAULT_PULSE_TRANSITION);
        return;
      case 'thinking':
      case 'connecting':
      case 'initializing':
        // Calmer "thinking": a slow shimmer between dim and modest, never near the white-bloom range.
        setSpeed(26);
        animateScale(0.3, DEFAULT_TRANSITION);
        animateAmplitude(0.5, DEFAULT_TRANSITION);
        animateFrequency(1, DEFAULT_TRANSITION);
        animateBrightness([0.7, 1.25], DEFAULT_PULSE_TRANSITION);
        return;
      case 'speaking':
        setSpeed(70);
        animateScale(0.3, DEFAULT_TRANSITION);
        ...
        animateBrightness(1.25, DEFAULT_TRANSITION);
        return;
    }
